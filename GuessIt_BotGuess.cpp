...
void gan(int number)
{
    ...
    while(l <= r)
    {
        ...
        maydoan[++cntmaydoan] = g;

        if(g<number)l = g + 1;
        else if(g>number) r = g - 1;
        else break;
    }
}
...
int main(int argc, char* argv[])
{
    ...
    int id = 1;
    do {
        displayPlayerGuess(maydoan[id]);
        updateAnswer(answer, markedNumbers, maydoan[id], number);
...
void load_SDL_and_Images()
{
    ...
    gameImg = loadTexture("images/maydoan.bmp", renderer);
...
void printStartMsg()
{
    renderTexture(startMsg, renderer, 296, 100);
    SDL_RenderPresent(renderer);
}
